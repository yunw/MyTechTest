package com.test.example.network.crawler;
...
public class SearchCrawler implements Runnable {
	...
	// 解析页面并找出链接
	private ArrayList<String> retrieveLinks(URL pageUrl, String pageContents, Set<String> crawledList, boolean limitHost) {
		// 用正则表达式编译链接的匹配模式。
...
//			System.out.println("已处理的URL：" + url);
			crawledSet.add(url);
			String pageContents = downloadPage(verifiedUrl);

			if (pageContents != null && pageContents.length() > 0) {
				// 从页面中获取有效的链接
				ArrayList<String> links = retrieveLinks(verifiedUrl, pageContents, crawledSet, limitHost);

				toCrawlList.addAll(links);
				if (searchStringMatches(pageContents, searchString, caseSensitive)) {
					result.add(url);
					System.out.println("符合条件的url：" + url + "\n\n");
				}
			}
	...
	public static void main(String[] args) {
		...
		System.out.println("searhString: " + args[2]);
