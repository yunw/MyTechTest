...
/**
 * 快速排序 对冒泡排序的一种改进，若初始记录序列按关键字有序或基本有序，蜕化为冒泡排序。使用的是递归原理，在所有同数量级O(n longn)
 * 的排序方法中，其平均性能最好。就平均时间而言，是目前被认为最好的一种内部排序方法
 * 基本思想是：通过一躺排序将要排序的数据分割成独立的两部分，其中一部分的所有数据都比另外一部分的所有数据都要小
 * ，然后再按此方法对这两部分数据分别进行快速排序，整个排序过程可以递归进行，以此达到整个数据变成有序序列。 三个指针:
 * 第一个指针称为pivotkey指针（枢轴
 * ），第二个指针和第三个指针分别为left指针和right指针，分别指向最左边的值和最右边的值。left指针和right指针从两边同时向中间逼近
 * ，在逼近的过程中不停的与枢轴比较，将比枢轴小的元素移到低端，将比枢轴大的元素移到高端，枢轴选定后永远不变，最终在中间，前小后大。
 * 
 * 需要两个函数：
 * 
...
public class QuickSort {
	public static void main(String[] args) {
		...
		int[] array = { 49, 38, 65, 97, 76, 13, 27,100,304,203,999,123,56,34,991234,234,1,678,8765,345,289,109,3443,111,789,45678903,24567,744 };
